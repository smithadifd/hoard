...
This is the architecture "why" page. If you're returning after a few months and want to reload context, or you're evaluating whether the architecture makes sense before self-hosting, this covers the reasoning behind the non-obvious choices. For the "what" — data flow, tables, the scheduler — see [Architecture](/architecture/). For the *product* "why" — what Hoard is for and what it refuses to become — see [Product philosophy](/product-philosophy/).
