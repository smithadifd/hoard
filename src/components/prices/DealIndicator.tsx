...
export function DealIndicator({ rating, score, compact = false, lowConfidence = false }: DealIndicatorProps) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-bold text-white ${config.bg}${lowConfidence ? ' border border-dashed border-white/40' : ''}`}
