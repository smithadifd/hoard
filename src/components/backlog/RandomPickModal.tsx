    ...
    <Dialog.Root open={open} onOpenChange={(next) => { if (!next) onClose(); }}>
      <Dialog.Portal>
        ...
        <Dialog.Content
          ...
          onInteractOutside={(e) => {
            // Only allow closing by clicking backdrop when animation has landed
            if (phase !== 'landed') e.preventDefault();
          }}
          onEscapeKeyDown={(e) => {
            // Prevent Escape during animation
            if (phase !== 'landed') e.preventDefault();
          }}
          className="fixed left-1/2 top-1/2 z-50 -translate-x-1/2 -translate-y-1/2 w-full max-w-md mx-4 bg-card border border-white/[0.08] rounded-lg shadow-xl overflow-hidden"
        >
          <Dialog.Title className="sr-only">
            {phase === 'landed' ? `Your pick: ${game.title}` : 'Picking a random game…'}
          </Dialog.Title>
